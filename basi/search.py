...
def searchColumns(df,key,strict=False):
    if strict:
        result=df.apply(lambda row: row.astype(str).str.match(key)).any()
    else:
...
def searchSeries(row,key,strict=False):
    if strict:
        res=[index for index,k in row.astype(str).str.match(key).items() if k]
    else:
...
def searchDataFrame(df,key,n=0,strict=False):
    ...
    if columns is not None:
        column=columns[0]
        index=searchSeries(df[column],key,strict)
